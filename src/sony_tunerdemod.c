...
static sony_result_t X_tune1 (sony_tunerdemod_t * pTunerDemod, sony_dtv_system_t system, uint32_t freqKHz, sony_dtv_bandwidth_t bandwidth,
                              uint8_t isCable, int32_t shiftFrequencyKHz)
{
    uint8_t data[11] = {0};
...
static sony_result_t X_tune2 (sony_tunerdemod_t * pTunerDemod, sony_dtv_bandwidth_t bandwidth,
                              sony_tunerdemod_clockmode_t clockMode, int32_t shiftFrequencyKHz)
{
    uint8_t data[3] = {0};
    ...
    if (shiftFrequencyKHz != 0) {
        ...
        data[0] = (uint8_t)(((uint32_t)shiftFreq >> 8) & 0xFF);
        data[1] = (uint8_t)((uint32_t)shiftFreq & 0xFF);
        ...
        data[0] = (uint8_t)((uint32_t)shiftFreq & 0xFF);
...
sony_result_t sony_tunerdemod_CommonTuneSetting1 (sony_tunerdemod_t * pTunerDemod, sony_dtv_system_t system,
                                                  uint32_t frequencyKHz, sony_dtv_bandwidth_t bandwidth,
    ...
    /* Clock mode selection */
    {
        ...
        if (((system == SONY_DTV_SYSTEM_ISDBT) || (system == SONY_DTV_SYSTEM_ISDBTSB))
            ...
            if (newClockMode == SONY_TUNERDEMOD_CLOCKMODE_UNKNOWN) {
                ...
                uint32_t Fl = 0;
                                      /*  A        B        C   */
                uint32_t PLLCLK[] =   {1152000, 1120000, 1152000};
                 ...
                 */
                for (i = 0; i < 3; i++) {
                    uint32_t Vu = (Fu * DIV_DEMOD[i] + PLLCLK[i] - 1) / PLLCLK[i];
                    uint32_t Vl = (Fl * DIV_DEMOD[i] + PLLCLK[i] - 1) / PLLCLK[i];
                                         ...
                                         int32_t value)
{
    ...
    case SONY_TUNERDEMOD_CONFIG_FIXED_CLOCKMODE:
        if ((value < (int32_t)SONY_TUNERDEMOD_CLOCKMODE_UNKNOWN) || (value > (int32_t)SONY_TUNERDEMOD_CLOCKMODE_C)) {
             SONY_TRACE_RETURN (SONY_RESULT_ERROR_RANGE);
        }
...
sony_result_t sony_tunerdemod_SetRFLevelCompensation (sony_tunerdemod_t * pTunerDemod,
                                                      sony_result_t (* pRFLevelCompensation) (sony_tunerdemod_t *, int32_t *))
{
