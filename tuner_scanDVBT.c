 ...
 * Modified by William Ho  2016-10-05
 *
...
static int ReadArgv(int argc, char *argv[]) {
	...
	while ((c = getopt_long(argc, argv, short_options, long_options, NULL)) != -1) {
		...
		default:
			...
			printf(
					"Usage %s [-d DTV_standard] [-s Star_frequency_kHZ] "
					"[-s End_frequency_kHZ] [-b bandwidth_MHz] [-p] [-j export_filename ][-h] \n",
					argv[0]);
			printf("\tExample: %s -d 1 -s 533000 -e 596000 -b 6 -j channel.json -p\n", argv[0]);
	...
	printf("Run app as: type= %s, Star-freq= %d kHz, End-freq= %d kHz, BandWidth= %d MHz, Digging prog=%s ", DTV_STANDARD_STR[g_dtv],
			g_StartFreq, g_EndFreq, g_BandWidth, g_EnableDigProg?"Enable":"Disable");
...
int main(int argc, char *argv[]) {
	...
	/* Set DVB Parameter */
	if (g_dtv == DVB_T2) {
		param.dvbSytem = SYSTEM_DVBT2;
	} else {
		param.dvbSytem = SYSTEM_DVBT;
	}
	...
	//Before prepare channel, we have to setup DVB system.
	controller.tunerDemod.system = (g_dtv == DVB_T2)?SONY_DTV_SYSTEM_DVBT2:SONY_DTV_SYSTEM_DVBT;
	result = PrepareForChannel(&controller);
	if (result == SONY_RESULT_OK) {
		printf("Device PrepareForChannel done.\n");
	}else{
		printf("Error : PrepareForChannel failed. (result = %s)\n", Common_Result[result]);
				return -1;
	}
...

	{
		...
		if (controller.tunerDemod.diverMode == SONY_TUNERDEMOD_DIVERMODE_MAIN) {
			printf("-----|--------|--------|-----|---------|---------|---------|-----|-----+-----|------|\n");
			printf("  %%  | SYSTEM |  FREQ  | PLP | PROFILE | TS LOCK | SNR(dB) | SQI | SSI  (Sub)|RF(db)|\n");
			printf("-----|--------|--------|-----|---------|---------|---------|-----|-----+-----|------|\n");
		} else {
			printf("-----|--------|--------|-----|---------|---------|---------|-----|-----|------|\n");
			printf("  %%  | SYSTEM |  FREQ  | PLP | PROFILE | TS LOCK | SNR(dB) | SQI | SSI |RF(db)|\n");
			printf("-----|--------|--------|-----|---------|---------|---------|-----|-----|------|\n");
		}
		...
		if (g_dtv == DVB_T2) {
			scanParam.system = SONY_DTV_SYSTEM_DVBT2;
			scanParam.t2Profile = SONY_DVBT2_PROFILE_ANY;
		} else {
			//Default call DVB_T scanner
			scanParam.system = SONY_DTV_SYSTEM_DVBT;
		}
		result = sony_integ_dvbt_t2_Scan(&controller.tunerDemod, &scanParam, &dvbt_t2ScanCallbackHandler);
		printf("\n - Result: %s\n", Common_Result[result]);
	}
	...
	if (!g_EnableDigProg)
		return 0;
	//Here we start to dig program data in every chennel.
	printf("Tuner scanner done, here we will dig %d channel(s)\n\n",g_FoundChNum);
	result = FindChannelsProg();
	if (result != SONY_RESULT_OK) {
